...
function parseEditorial(description: string, childNames: string[] = []): EditorialSection[] {
  ...
  let cleaned = description
    .replace(/n{4,}/g, '\n')   // 4+ consecutive n's anywhere → newline
    .replace(/\nn{2,}/g, '\n') // newline + 2+ n's → newline
    .replace(/^n+/, '')        // strip leading n's
    ...
    <div className="grain-overlay min-h-screen">
      ...
      {allDrinks.length > 0 && (
        <section id="references" className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-12 lg:py-20">
          ...
          {/* Reference grid — div-based layout, no <table> elements */}
          <div style={{ borderRadius: '12px', border: '1px solid #E5DDD5', overflow: 'hidden' }}>
            {/* Header row */}
            <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 0.8fr 0.8fr 2fr 0.8fr', backgroundColor: '#F0EBE3', borderBottom: '1px solid #E5DDD5' }}>
              {[tCategory('table_brand'), tCategory('table_origin'), tCategory('table_vintage'), tCategory('table_type'), tCategory('table_restaurants'), tCategory('table_price')].map((header, idx) => (
                <div key={idx} style={{ padding: '14px 18px', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.15em', color: '#888' }}>{header}</div>
              ))}
            </div>
            {/* Data rows */}
            {allDrinks.map((drink, i) => {
              ...
              return (
                <div key={drink.id} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 0.8fr 0.8fr 2fr 0.8fr', backgroundColor: i % 2 === 0 ? '#FFFFFF' : '#FAFAF7', borderBottom: '1px solid rgba(229,221,213,0.3)' }}>
                  <div style={{ padding: '14px 18px', fontWeight: 600, color: '#2C1810' }}>{drink.name}</div>
                  <div style={{ padding: '14px 18px', fontSize: '13px', color: 'rgba(44,24,16,0.6)' }}>{drink.country ?? '—'}{drink.region ? ` – ${drink.region}` : ''}</div>
                  <div style={{ padding: '14px 18px', fontSize: '13px', color: 'rgba(44,24,16,0.6)' }}>{drink.vintage ? `${drink.vintage}` : '—'}</div>
                  <div style={{ padding: '14px 18px', fontSize: '13px', color: 'rgba(44,24,16,0.6)' }}>{drink.appellation ?? '—'}</div>
